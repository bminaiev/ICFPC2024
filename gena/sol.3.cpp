  ...
  while (1.0 * clock() / CLOCKS_PER_SEC < TL) {
    ...
    if (it % 2 == 0) {
        ...
        for (int id = lft; id <= rgt; id++) {
          for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
              ...
              int mini_delta = 0;
              if (id < n - 1) {
                mini_delta -= Eval(pts[order[id]], pts[order[id + 1]], speeds[order[id]], speeds[order[id + 1]]);
              }
              mini_delta -= Eval(pts[order[id - 1]], pts[order[id]], speeds[order[id - 1]], speeds[order[id]]);
              auto new_ds = Point(dx, dy);
              if (id < n - 1) {
                mini_delta += Eval(pts[order[id]], pts[order[id + 1]], speeds[order[id]] + new_ds, speeds[order[id + 1]]);
              }
              mini_delta += Eval(pts[order[id - 1]], pts[order[id]], speeds[order[id - 1]], speeds[order[id]] + new_ds);
              if (mini_delta <= 0) {
                delta += mini_delta;
                if (mini_delta < 0) found = true;
                speeds[order[id]] = speeds[order[id]] + new_ds;
                ds[order[id]] = ds[order[id]] + new_ds;
              }
            }
          }
        }
      ...
      if (delta <= 0 || (delta > 0 && urd(rng) < exp(-1.0 * delta / cur_temp))) {
        ...
        for (int id = lft; id <= rgt; id++) {
          speeds[order[id]] = speeds[order[id]] - ds[order[id]];
        }
