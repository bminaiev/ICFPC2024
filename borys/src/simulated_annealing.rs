...
impl SaveChecker {
    ...
    pub fn should_save(&mut self, score: f64, search_for: SearchFor, time_ms: f64) -> bool {
...
impl SimulatedAnnealing {
        ...
        f64: From<T>,
    {
        ...
        let new_score: f64 = new_score.into();

        let delta_if_positive_is_good = {
            let delta: f64 = new_score - prev_score;
        ...
        if delta_if_positive_is_good >= (0.0) {
        ...
        let accept_probability =
            std::f64::consts::E.powf(delta_if_positive_is_good / self.current_temperature);
        assert!(accept_probability <= 1.0 + 1e-9);
        assert!(accept_probability >= 0.0);

        if self.rnd.gen_range(0.0..1.0) <= accept_probability {
            self.last_score = new_score;
            self.accept_rate.add(self.iterations_passed);
            true
        } else {
