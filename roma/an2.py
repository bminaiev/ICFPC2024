...
def parse_tokens(tokens: List[str]) -> ICFPValue:
    ...
    if indicator == 'T':
        ...
        return Application(
            Lambda(0, Application(Variable(-1), Variable(0)), {-1: Thunk(create_unary_op(op), {})}),
            arg
        )
    elif indicator == 'B':
        op = body
...
def create_unary_op(op: str) -> Lambda:
    return Lambda(0, Application(Variable(-1), Variable(0)), {-1: Thunk(Lambda(0, create_unary_op_body(op), {}), {})})

def create_unary_op_body(op: str) -> ICFPValue:
    if op == '-':
        return Lambda(0, Integer(-Variable(0).value), {})
    elif op == '!':
        return Lambda(0, Boolean(not Variable(0).value), {})
    elif op == '#':
        return Lambda(0, Integer(base94_to_int(Variable(0).value)), {})
    elif op == '$':
        return Lambda(0, String(int_to_base94(Variable(0).value)), {})
    else:
        raise ValueError(f"Unknown unary operator: {op}")

def create_binary_op(op: str) -> Lambda:
    return Lambda(0, Lambda(1, Application(Application(Variable(-1), Variable(0)), Variable(1)), {}), {-1: Thunk(Lambda(0, Lambda(1, create_binary_op_body(op), {}), {}), {})})

def create_binary_op_body(op: str) -> ICFPValue:
    if op in '+-*/%<>=':
        return create_arithmetic_op_body(op)
    elif op in '|&':
        return create_boolean_op_body(op)
    elif op == '.':
...
def create_arithmetic_op_body(op: str) -> ICFPValue:
...
def create_boolean_op_body(op: str) -> ICFPValue:
    op_map = {'|': lambda x, y: x or y, '&': lambda x, y: x and y}
    return Lambda(0, Lambda(1, Boolean(op_map[op](evaluate(Variable(0), {}).value, evaluate(Variable(1), {}).value)), {}), {})
...
def evaluate(expr: ICFPValue, env: Dict[int, Thunk]) -> ICFPValue:
    if isinstance(expr, (Boolean, Integer, String)):
        ...
        if isinstance(func, Lambda):
            return evaluate(func.body, {**func.env, func.var_num: Thunk(expr.arg, env)})
        else:
