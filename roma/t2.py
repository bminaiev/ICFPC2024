...
def extract_lambda_body(tokens):
    ...
    while depth > 0 and tokens:
        ...
        if token.startswith('L') or token.startswith('B') or token.startswith('?'):
            depth += 1
        elif token == 'T' or token == 'F' or token.startswith('I') or token.startswith('S'):
            pass
        else:
            depth -= 1
    return body_tokens
...
def evaluate(tokens, env=None):
    ...
    if indicator == 'T':
        ...
        return lambda arg: evaluate(lambda_body.copy(), {**current_env, variable_number: arg})
    elif indicator == 'v':
        variable_number = decode_base94(body)
